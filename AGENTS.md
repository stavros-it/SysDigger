...
| `tools.py` | ~1710 | Tool catalogue: 4 categories, 27 tools, 59 PowerShell modes (includes Autopilot hash with validation, disk analyzer with 8 modes incl. scan-then-pick cleanup, memory diagnostic, hosts editor, WU trigger, appx manager, dev cache cleaner, hibernate manager, UEFI BIOS reboot, MTP/Android USB reset) |
