...
| `tools.py` | ~2520 | Tool catalogue: 4 categories, 30 tools, 70 PowerShell modes (includes Autopilot hash with validation, disk analyzer with 8 modes incl. scan-then-pick cleanup, appx manager, installer cleanup (orphaned .msi/.msp quarantine), dev cache cleaner incl. jcode data removal, hibernate manager, UEFI BIOS reboot, MTP/Android USB reset, SATA/AHCI controller reset, disk status/online, disk rescue) |
