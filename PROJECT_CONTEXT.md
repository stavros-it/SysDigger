...
- **Windows Tools** — 29 maintenance utilities across 4 categories (System Repair, Maintenance, Hardware & Diagnostics, System Info & Status) powered by PowerShell subprocess execution with live-streamed output: SFC, DISM (Clean/Repair), WinRE Manager, Cleanup, Disk Analyzer (large files / top folders / recursive folder size map / duplicate file finder + scan-then-pick cleanup of biggest AppData folders or user profile files), Appx Manager (uninstall Appx packages by size), Dev Cache Cleaner (npm/pip cache + uv / LM Studio / Vortex / RSI Launcher updater folders + jcode data), Hibernate Manager (enable/disable), Reset Spooler, Install/Uninstall repair, Flush Network (6 modes), Trim SSD, Check HDD, Disk Status (list status / bring disk online), Disk Rescue (failing-disk mapper + bad-aware copier with adjustable scan step numbers: List Disks / Scan Map / Map Report / Bad-Aware Copy / Lost Files), Device Query (5 modes — HID services, remove HID errors, reset Bluetooth adapter, reset MTP/Android USB, reset SATA/AHCI controllers), Services Check, Memory Diagnostic (GUI + schedule on reboot), Time Sync, Activation, Wi-Fi, Firewall, Power & System, Autopilot Hash, Hosts File Editor, Windows Update (scan + install), UEFI BIOS Reboot (shutdown /r /fw /t 0)
...
├── tools.py              # Windows maintenance tools catalogue: 4 categories / 30 tools / 70 modes (PowerShell scripts ported verbatim from `tools source/`, plus v4.11 scan-then-pick cleanup scripts, the v4.19 Disk Rescue engine and the v4.20 installer orphan quarantine)
...
- 4 categories, 30 tools, 70 modes total
